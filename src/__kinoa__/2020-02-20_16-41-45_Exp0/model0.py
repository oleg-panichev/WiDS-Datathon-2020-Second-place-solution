...
random.seed(script_id)
np.random.seed(script_id)
...
for c in est_cols:
    ...
    if True: #c['fillna']:
        ...
        train.loc[train[c['name']].isnull(), c['name']] = train.loc[train[c['name']].isnull(), c['name'] + '_est']
        test.loc[test[c['name']].isnull(), c['name']] = test.loc[test[c['name']].isnull(), c['name'] + '_est']
...
# dummy_cols = []
# for c in cat_features:
#     if c in df_all.columns and '_id' not in c and 'bmi' not in c and df_all[c].nunique(dropna=False) > 2:
#         dummy_cols.append(c)
dummy_cols = ['ethnicity', 'gender', 'hospital_admit_source', 'icu_admit_source', 'icu_stay_type', 'icu_type', 'arf_apache', 'gcs_unable_apache', 'intubated_apache', 'ventilated_apache', 'aids', 'cirrhosis', 'diabetes_mellitus', 'hepatic_failure', 'immunosuppression', 'leukemia', 'lymphoma', 'solid_tumor_with_metastasis', 'apache_3j_bodysystem', 'apache_2_bodysystem']
dprint(f'dummy_cols: {dummy_cols} ({len(dummy_cols)})')
df_all = pd.get_dummies(df_all, dummy_na=True, columns=dummy_cols)
# df_all = pd.concat([df_all, dummies], axis=1)

# Split back on train and test
...
for fold_i, (train_index, valid_index) in enumerate(kf.split(X, y)):
    ...
    if train_lrsgd:
        ...
        ss = StandardScaler()
        x_train_ss = ss.fit_transform(x_train.fillna(0))
        x_valid_ss = ss.transform(x_valid.fillna(0))
        x_test_ss = ss.transform(x_test.fillna(0))

        model.fit(x_train_ss, y_train, validation=(x_valid_ss, y_valid))

        p_lrsgd = model.predict(x_valid_ss)#[:, 1]
        p_lrsgd = np.clip(p_lrsgd, 0, 1)
        p_valid.append(p_lrsgd)

        err = roc_auc_score(y_valid, p_lrsgd)
        dprint('{} LRSGD AUC: {:.6f}'.format(fold_i, err))

        p_lrsgd_test = model.predict(x_test_ss)
        p_lrsgd_test = np.clip(p_lrsgd_test, 0, 1)
        p_test.append(p_lrsgd_test)
